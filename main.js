...
let gameStarted = false;                            // 클릭하는 순간 true가 됨
const BG_MOVING_SPEED = 5;                          // x 좌표를 5씩 줄일 거다
let bgX = 0;
let scoreText = document.getElementById("score");   // score라는 id select
let score = 0;
 ...
 */
function animate() {
    if (gameOver) {     // gameOver되면 멈춰!
      return;
    }
  
    bgmSound.play();    // 백그라운드 sound 출력
    ...
    obstacleArray.forEach((a) => {
      ...
      if (a.x < -OBSTACLE_WIDTH) {
        ...
        scoreSound.play();    // score sound 출력
        setTimeout(() => {
    ...
    rtan.draw();        // 위에서 만든 rtan 객체 draw
    timer++;
    requestAnimationFrame(animate);
  }

  /**
 * Space 클릭 시 이벤트 (점프 기능)
 */
 ...
 * 충돌 체크 함수
 */
function collision(first, second) {     // first, second가 부딪히는가?
    return !(
